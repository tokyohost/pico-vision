/*
  ...
  本版将裸屏固定方式改为“FPC侧固定插入卡钩 + 对侧双 M2 螺柱压条”：
    ...
    5. 取消 FPC 一侧两颗屏幕螺柱和原 X 形背压板；
    6. FPC 一侧改为左右两个固定插入卡钩，装配时先斜插再放平；
    7. 非 FPC 一侧保留两颗 M2 螺柱，使用独立直条压板轻压屏幕后壳；
    8. 压板仅通过两个小压点接触屏幕后壳，建议贴 0.2~0.3mm EVA 泡棉；
    9. FPC 根部避让槽高度缩至 3.60mm，排线折向后盖方向；
    10. 整套屏幕结构向 FPC 侧移动 1.92mm，使 VA 开窗位于外壳几何中心；
    11. 外壳外形缩为 50.00 x 72.40 x 18.00mm；
    12. 50.40mm 高的开窗在 72.40mm 外壳上形成上下各约 11.00mm 的等宽边框；
    13. Pico、Type-C、右侧三按键、后盖和 Pico 小压盖结构按新外壳高度自动重算；
    14. 卡钩为固定钩，不依赖反复弯折，PLA/PETG 均可打印。

  装屏顺序：
    - 将 FPC 向屏幕后方预折；
    - 先把 FPC 一侧屏幕边缘斜插到两个固定卡钩下；
    - 再将另一侧放入凹槽；
    - 最后用两颗 M2 螺丝锁紧直条压板，均匀轻压即可。
  ...
  part 可选：
    "print_plate"：前壳、后盖、屏幕直条压板、两块 Pico 小压盖和侧按键压板同板打印
    "front_shell"：只导出带加深凹槽、FPC侧卡钩和对侧双螺柱的前壳
    "screen_clamp_bar"：只导出屏幕直条压板
    "screen_x_backplate"：兼容旧名称，同样导出屏幕直条压板
    "back_cover"：只导出后盖
...
$fn = 56;
part = "print_plate";    // 可选："print_plate", "front_shell", "screen_clamp_bar", "screen_x_backplate", "back_cover", "pico_pair_clamp_cover", "side_button_plate", "button_plate", "typec_clamp_cover", "typec_fit_test", "assembly", "exploded"
...
// 独立直条压板。压板主体偏向屏幕内侧，螺丝孔位于靠外一侧。
screen_clamp_bar_w = 41.00;
screen_clamp_bar_h = 12.40;
screen_clamp_bar_t = 1.60;
screen_clamp_bar_r = 1.00;
screen_clamp_bar_center_y = screen_mount_y + 29.80;
...
// -Y FPC 侧两个固定插入卡钩。卡钩不需要弹性变形，先斜插屏幕再放平。
screen_hook_x = 15.50;
screen_hook_w = 5.20;
screen_hook_overhang = 0.75;
screen_hook_clearance = 0.15;
screen_hook_lip_t = 0.80;
screen_hook_lead = 0.35;
screen_hook_base_t = screen_recess_guide_t;
screen_hook_inner_y = screen_mount_y - screen_recess_h/2;
screen_hook_lip_z0 = screen_back_z + screen_hook_clearance;
