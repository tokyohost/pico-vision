/*
  ...
  V4修正：FPC转接PCB为20x30mm，20mm短边朝FPC端；四孔柱距改为X=15.00mm、Y=18.80mm。
  ...
  V4 同步适配新的 20 x 30mm FPC 转接 PCB 四孔布局：
    ...
    16. Pico、Type-C、右侧三按键和后盖结构按新外壳高度自动重算；
    17. 右侧三按键整体向 PCB 尾部（+Y）移动 5.00mm，避开 Type-C 侧固定结构；
    18. 取消后盖 PCB 尾部两颗固定螺柱及两块 L 形小压盖，仅保留 Type-C 侧双螺柱 EVA 横压板；
    19. 卡槽根部加宽到8.00mm、扣唇加厚到1.20mm、有效搭接增加到1.80mm；
    20. 卡钩为固定钩，不依赖反复弯折，PLA/PETG 均可打印。
  ...
  part 可选：
    "print_plate"：前壳、后盖、屏幕异形盖板、ESP32 EVA横压板和侧按键压板同板打印
...
$fn = 56;
part = "print_plate";    // 可选："print_plate", "front_shell", "screen_clamp_bar", "screen_x_backplate", "back_cover", "esp32_usb_clamp_bar", "side_button_plate", "button_plate", "typec_clamp_cover", "typec_fit_test", "assembly", "exploded"
...
side_button_center_y = side_button_shift_to_tail; // 整组向PCB尾部(+Y)移动5.00mm
...
// 按键向尾部移动后会与尾部止推块在Y方向重叠，因此改用实际X方向净距校验。
esp32_tail_stop_to_button_x_gap =
    side_button_plate_inner_x
    - (esp32_tail_stop_x + esp32_tail_stop_w/2);
...
echo(str("校验：ESP32固定件对按键避让：Type-C横梁X间隙=",
         ...
         "mm；PCB尾部螺柱/小压盖=已取消；尾部止推块X间隙=",
         esp32_tail_stop_to_button_x_gap, "mm"));
...
assert(esp32_tail_stop_to_button_x_gap >= 1.50,
       "错误：按键向尾部移动后，侧按键压板与尾部止推挡块X间隙不足");
