/*
  ...
  本版根据 42.72 x 58.80mm、无 PCB 的插接 10Pin 裸屏图纸改造：
    ...
    5. 屏幕下边中央保留 20.40mm 宽 FPC 弯折/出线通道；
    6. 前盖增加上、下两对共四个 M2 螺柱，螺柱全部位于屏幕主体外侧；
    7. 新增独立 X 形屏幕背压板，四颗 M2 螺丝把裸屏压紧在前盖凹槽中；
    8. X 背板只通过四个小压点接触屏幕后壳，主体与屏幕背面保留间隙；
    9. X 背板中央和 FPC 下方保持开放，避免整片压板积热或夹伤排线；
    10. 原 Pico、Type-C、右侧三按键、后盖和小压盖结构保持不变；
    11. 外壳外形继续保持 52.00 x 77.20 x 18.00mm，便于沿用现有后盖和内部布局；
    12. 建议 X 背板四个压点贴 0.2~0.4mm EVA/泡棉胶，螺丝只需均匀轻锁。
...
module front_screen_recess_cut() {
  // 从前面板内侧向正面削出 0.70mm 深的浅凹槽。
  // 凹槽底面承托裸屏正面，四周台阶负责 XY 定位。
  rounded_cut(
    screen_recess_w,
    screen_recess_h,
