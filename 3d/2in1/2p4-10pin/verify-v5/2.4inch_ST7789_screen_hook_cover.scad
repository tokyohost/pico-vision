/*
  ...
  本版将裸屏固定方式改为“对侧插入式盖板卡钩 + 原螺丝侧双 M2 锁紧”：
    ...
    7. 盖板右侧镂空三枚侧按键机构，FPC 端中央镂空排线折弯区；
    8. 盖板中间保持完整实心，不再设置中央减重/散热镂空；
    9. 从 FPC 侧卡扣边缘向屏幕中心 23.50mm 设置 FPC 小板固定位；
    10. FPC 小板改用两根 φ2.50mm 无孔实心圆柱，位于中心线 X=±5.20mm，中心距 10.40mm；
    11. 盖板用四个小压点轻压屏幕后壳，建议贴 0.2~0.3mm EVA 泡棉；
    12. FPC 根部避让槽高度缩至 3.60mm，排线折向后盖方向；
    13. 整套屏幕结构向 FPC 侧移动 1.92mm，使 VA 开窗位于外壳几何中心；
    14. 外壳外形缩为 50.00 x 72.40 x 18.00mm；
    15. 50.40mm 高的开窗在 72.40mm 外壳上形成上下各约 11.00mm 的等宽边框；
    16. Pico、Type-C、右侧三按键、后盖和 Pico 小压盖结构按新外壳高度自动重算；
    17. 卡槽根部加宽到8.00mm、扣唇加厚到1.20mm、有效搭接增加到1.80mm；
    18. 卡钩为固定钩，不依赖反复弯折，PLA/PETG 均可打印。

  装屏顺序：
    - 先将裸屏平放入凹槽，FPC 向后盖方向预折；
    - 将盖板 -Y 端的两枚插片推入对侧卡槽；
    - 确认 FPC 从中央缺口穿出，右侧缺口不干涉三枚按键；
    - 最后放下 +Y 螺丝侧，用两颗 M2 均匀轻锁。

  坐标方向：
    X = 屏幕宽度方向，+X 为右侧按键所在长边；
    Y = 屏幕高度方向，+Y 为保留双螺柱压条的一侧，-Y 为 FPC/固定卡钩一侧；
    Z = 前面板外表面指向后盖。

  part 可选：
    "print_plate"：前壳、后盖、屏幕异形盖板、ESP32 EVA横压板、两块尾部小压盖和侧按键压板同板打印
    "front_shell"：只导出带加深凹槽、对侧卡槽和原螺丝侧双螺柱的前壳
    "screen_clamp_bar"：只导出屏幕异形盖板
    "screen_x_backplate"：兼容旧名称，同样导出屏幕异形盖板
    "back_cover"：只导出后盖
    "esp32_edge_clamp_cover"：只导出一块 ESP32 边缘小压盖
    "esp32_usb_clamp_bar"：只导出 Type-C 后方的 ESP32 EVA 横压板
    "pico_pair_clamp_cover"：兼容旧名称，同样导出 ESP32 边缘小压盖
...
$fn = 56;
part = "print_plate";    // 可选："print_plate", "front_shell", "screen_clamp_bar", "screen_x_backplate", "back_cover", "esp32_edge_clamp_cover", "esp32_usb_clamp_bar", "pico_pair_clamp_cover", "side_button_plate", "button_plate", "typec_clamp_cover", "typec_fit_test", "assembly", "exploded"
...
// 为让正面横屏左右边框等宽，把整个裸屏安装中心向 FPC(-Y)侧移动 1.92mm。
// VA 本身相对玻璃中心向 +Y 偏 1.92mm，因此两者抵消后开窗中心回到外壳 Y=0。
screen_mount_y = -screen_aa_y_offset;          // -1.92
...
screen_window_y_offset =
    screen_mount_y + screen_aa_y_offset + screen_window_y_extra_tune; // 0.00
...
screen_clamp_screw_y = screen_mount_y + screen_clamp_screw_y_offset;
screen_boss_od = 5.00;
...
// -Y 对侧两个加强盖板卡槽。增加根部宽度、扣唇厚度和插入搭接量。
screen_hook_x = 16.00;
screen_hook_w = 7.00;
...
screen_hook_base_t = 1.60;
screen_hook_inner_y = screen_mount_y - screen_recess_h/2;
...
// 四个独立压点，分别靠近卡槽侧与螺丝侧；建议贴 0.2~0.3mm EVA。
screen_clamp_pad_x = 12.50;
screen_clamp_pad_y = screen_mount_y + 25.50;
screen_cover_hook_pad_x = 14.50;
screen_cover_hook_pad_y = screen_mount_y - 25.50;
screen_clamp_pad_d = 4.50;
screen_cover_under_gap = 0.55;
screen_clamp_pad_h = screen_cover_under_gap;
...
// 盖板避让：右侧三按键机构、-FPC 端排线折弯区以及中央减重散热窗。
screen_cover_button_cut_w = 6.00;
screen_cover_button_cut_h = 34.00;
screen_cover_button_cut_y = 0.00; // 与三按键组中心对齐
...
// FPC 卡口/转接小板固定位。
// Y 基准是盖板在 FPC 侧插入卡扣的边缘，向屏幕中心(+Y)量 23.50mm。
screen_fpc_board_from_hook = 23.50;
screen_fpc_board_mount_y =
    screen_cover_hook_end_y + screen_fpc_board_from_hook;
screen_fpc_board_boss_dx = 10.40;
screen_fpc_board_boss_x = 5.20; // 中心线左右各 5.2mm
screen_fpc_board_boss_od = 2.50;
screen_fpc_board_boss_h = 3.20;
...
screen_hook_base_to_end_wall_gap =
    screen_inner_half_y + screen_hook_inner_y - screen_hook_base_t;
screen_cover_right_rail_w =
    screen_clamp_bar_w/2 - screen_cover_button_cut_w
    - screen_cover_center_relief_w/2;
screen_fpc_board_boss_side_gap =
    screen_clamp_bar_w/2
    - (screen_fpc_board_boss_x + screen_fpc_board_boss_od/2);
screen_fpc_board_from_hook_actual =
    screen_fpc_board_mount_y - screen_cover_hook_end_y;
...
echo(str("校验：VA=", screen_aa_w, " x ", screen_aa_h,
         ...
         "mm；屏幕安装中心Y=", screen_mount_y,
         "mm；最终开窗中心Y=", screen_window_y_offset, "mm"));
...
echo(str("校验：屏幕异形盖板=", screen_clamp_bar_w, " x ",
         ...
         "mm；压点X=±", screen_clamp_pad_x,
         "mm；四压点Y=", screen_cover_hook_pad_y, "/",
         screen_clamp_pad_y, "mm；按键缺口=",
         screen_cover_button_cut_w, " x ", screen_cover_button_cut_h,
         "mm；FPC缺口=", screen_cover_fpc_cut_w, " x ",
...
echo(str("校验：对侧盖板卡槽X=±", screen_hook_x,
         ...
         "mm；卡槽高度间隙=", screen_hook_clearance, "mm"));
echo(str("校验：FPC小板螺柱中心=X±",
         screen_fpc_board_boss_x, "mm, Y=", screen_fpc_board_mount_y,
         "mm；距FPC侧卡扣边缘=",
         screen_fpc_board_from_hook_actual,
         "mm；中心距=", screen_fpc_board_boss_dx,
         "mm；实心圆柱=OD", screen_fpc_board_boss_od,
         " x H", screen_fpc_board_boss_h, "mm"));
...
assert(screen_hook_root_to_side_wall_gap > 1.00,
       "错误：加宽后的卡槽根部距离长边内壁过近");
assert(screen_hook_base_to_end_wall_gap > 0.80,
       "错误：加厚后的卡槽根部距离短边内壁过近");
assert(!enable_screen_cover_center_relief || screen_cover_right_rail_w > 2.00,
       "错误：按键缺口与中央散热窗之间的加强条过窄");
assert(screen_fpc_board_boss_side_gap > 2.00,
       "错误：FPC小板螺柱距离盖板左右边缘过近");
assert(abs(screen_fpc_board_from_hook_actual - 23.50) < 0.01,
       "错误：FPC小板螺柱距离卡扣边缘不是23.50mm");
...
// ---------- ESP32-S3 后盖固定：4螺柱 + 1块Type-C侧EVA横压板 + 2块尾部小压盖 ----------
...
// 尾部第二对螺柱移到PCB尾端外侧，两块L形压盖回伸到模组两侧的PCB窄边。
...
esp32_usb_bar_eva_pad_drop = 0.30;
...
esp32_boss_side_wall_gap =
    cover_w/2 - (esp32_tail_boss_x + esp32_mount_boss_od/2);
esp32_clamp_overlap =
    pico_pcb_w/2
    - (esp32_edge_clamp_toe_target_x - esp32_edge_clamp_toe_w/2);
esp32_clamp_preload =
    esp32_edge_clamp_toe_h - esp32_clamp_height_gap;
...
echo(str("校验：ESP32 Type-C侧螺柱=左(-", esp32_usb_boss_x,
         ",", esp32_usb_boss_y_left, ")/右(+", esp32_usb_boss_x,
         ",", esp32_usb_boss_y_right, ")mm；尾部螺柱=X±",
         esp32_tail_boss_x, ",Y=", esp32_mount_boss_y_tail,
         "mm；尾部L压盖搭住PCB窄边=",
         esp32_clamp_overlap, "mm；压紧预压=", esp32_clamp_preload,
         "mm；尾部模组距短边内壁=",
...
assert(esp32_boss_side_wall_gap > 2.00,
       "错误：ESP32固定螺柱距离后盖长边过近");
assert(esp32_clamp_overlap > 0.55 && esp32_clamp_overlap < 0.95,
       "错误：ESP32小压盖对PCB边缘的搭接量不合适");
assert(esp32_clamp_preload >= 0.05 && esp32_clamp_preload <= 0.15,
       "错误：ESP32小压盖的预压量不合适");
...
side_button_center_z = 10.75; // 整组向前面板移0.60mm，给后盖内表面留约0.55mm间隙
...
// 右侧壁键帽孔。
// 原键帽主帽外径为 φ8.00，因此前盖贯穿孔也按 φ8.00 设计，
// 尺寸目标为 φ8.00 ±0.03mm，让 φ8 主键帽能够进入前盖侧壁内。
// 注意：±0.03mm 是成品尺寸要求；普通 FDM 打印孔通常会偏小，需在切片补偿或试打印中校准。
button_keycap_hole_nominal_d = button_keycap_outer_d; // 8.00mm
button_keycap_hole_tol = 0.03;
button_keycap_hole_d = button_keycap_hole_nominal_d;

// 外口不再扩大为 φ6.50/其他直径，避免破坏 φ8.00 ±0.03mm 的孔口要求。
// 仅保留同直径的浅清边段，用于消除布尔共面和入口毛边。
button_keycap_entry_d = button_keycap_hole_d;
button_keycap_entry_depth = 0.20;
...
// 按键压板与ESP32后盖固定件的三维装配间隙。
side_button_plate_inner_x =
    side_button_plate_outer_x - side_button_plate_t;
side_button_plate_y_min =
    side_button_center_y - side_button_plate_w/2;
side_button_plate_y_max =
    side_button_center_y + side_button_plate_w/2;
side_button_plate_z_max =
    side_button_center_z + side_button_plate_h/2;
side_button_to_back_cover_z_gap =
    (shell_depth - cover_t) - side_button_plate_z_max;

esp32_usb_center_beam_to_button_x_gap =
    side_button_plate_inner_x - esp32_usb_bar_center_w/2;
esp32_usb_right_ear_to_button_y_gap =
    side_button_plate_y_min
    - (esp32_usb_boss_y_right + (esp32_mount_boss_od + 1.20)/2);
esp32_usb_right_boss_to_button_y_gap =
    side_button_plate_y_min
    - (esp32_usb_boss_y_right + esp32_mount_boss_od/2);
esp32_tail_boss_to_button_y_gap =
    (esp32_mount_boss_y_tail - esp32_mount_boss_od/2)
    - side_button_plate_y_max;
esp32_tail_clamp_to_button_y_gap =
    (esp32_mount_boss_y_tail
      + esp32_edge_clamp_toe_local_y
      - esp32_edge_clamp_arm_d/2)
    - side_button_plate_y_max;
esp32_tail_stop_to_button_y_gap =
    (esp32_tail_stop_y - esp32_tail_stop_t/2)
    - side_button_plate_y_max;
...
echo(str("校验：键帽φ", button_keycap_outer_d,
         ...
         "；侧壁贯穿孔φ", button_keycap_hole_d,
         " ±", button_keycap_hole_tol,
         "；允许范围=", button_keycap_hole_d - button_keycap_hole_tol,
         "~", button_keycap_hole_d + button_keycap_hole_tol,
         "mm；相邻键帽边缘间隙=", side_button_keycap_edge_gap, "mm"));

assert(abs(button_keycap_hole_d - button_keycap_outer_d) < 0.001,
       "错误：前盖按键孔直径必须与 φ8 键帽外径一致");
...
echo(str("校验：按键压板占用=X", side_button_plate_inner_x,
         "~", side_button_plate_outer_x, "mm, Y", side_button_plate_y_min,
         "~", side_button_plate_y_max, "mm；与后盖Z间隙=",
         side_button_to_back_cover_z_gap, "mm"));
echo(str("校验：ESP32固定件对按键避让：Type-C横梁X间隙=",
         esp32_usb_center_beam_to_button_x_gap,
         "mm；右螺丝耳Y间隙=", esp32_usb_right_ear_to_button_y_gap,
         "mm；右螺柱Y间隙=", esp32_usb_right_boss_to_button_y_gap,
         "mm；尾部螺柱Y间隙=", esp32_tail_boss_to_button_y_gap,
         "mm；L压盖Y间隙=", esp32_tail_clamp_to_button_y_gap,
         "mm；尾挡块Y间隙=", esp32_tail_stop_to_button_y_gap, "mm"));

assert(side_button_to_back_cover_z_gap >= 0.50,
       "错误：侧按键压板与后盖内表面Z间隙不足");
assert(esp32_usb_center_beam_to_button_x_gap >= 2.00,
       "错误：Type-C横压板中间横梁侵入侧按键压板X区域");
assert(esp32_usb_right_ear_to_button_y_gap >= 0.60,
       "错误：Type-C横压板右螺丝耳与侧按键压板Y间隙不足");
assert(esp32_usb_right_boss_to_button_y_gap >= 1.00,
       "错误：Type-C侧右螺柱与侧按键压板Y间隙不足");
assert(esp32_tail_boss_to_button_y_gap >= 1.50,
       "错误：ESP32尾部螺柱与侧按键压板Y间隙不足");
assert(esp32_tail_clamp_to_button_y_gap >= 0.45,
       "错误：ESP32尾部L压盖与侧按键压板Y间隙不足");
assert(esp32_tail_stop_to_button_y_gap >= 1.00,
       "错误：ESP32尾部止推挡块与侧按键压板Y间隙不足");
...
module at_esp32_mount_bosses() {
  ...
  // 尾部两颗螺柱均位于按键压板Y范围之外。
  for (sx = [-1, 1])
    translate([sx * esp32_tail_boss_x, esp32_mount_boss_y_tail, 0])
      children();
}
...
module esp32_usb_clamp_bar() {
  ...
  difference() {
    union() {
      ...
      // EVA粘贴平台：底面向PCB方向下沉0.30mm。
      add_box(
        esp32_usb_bar_eva_pad_w,
        esp32_usb_bar_eva_pad_h,
        esp32_usb_bar_eva_pad_drop,
        0,
        0,
        -esp32_usb_bar_eva_pad_drop
    ...
    translate([
      -esp32_usb_boss_x,
      0,
      -esp32_usb_bar_eva_pad_drop - 0.20
    ])
      cylinder(
        d = esp32_usb_bar_screw_d,
        h = esp32_usb_bar_t + esp32_usb_bar_eva_pad_drop + 0.60
      );
...
module esp32_edge_clamp_covers_installed_local() {
  if (enable_esp32_mount) {
    ...
    // 尾部第二对螺柱使用左右两块独立边缘小压盖。
    for (sx = [-1, 1])
      translate([
        sx * esp32_tail_boss_x,
        esp32_mount_boss_y_tail,
        cover_t + esp32_mount_boss_h
      ])
        esp32_edge_clamp_cover(sx);
  }
}
...
module side_button_hole_cuts() {
  if (enable_side_buttons)
    at_side_button_positions() {
      // φ8.00 主孔只贯穿右侧壁。
      // 内侧仅重叠 0.08mm，避免 φ8 切口继续侵入并削弱壳内的 6x6 开关限位框。
      translate([side_button_inner_x - 0.08, 0, 0])
        rotate([0, 90, 0])
          cylinder(d = button_keycap_hole_d,
                   h = wall + 0.16);

      // 外侧同为 φ8.00 的浅清边段，不做扩孔或沉孔。
      translate([
        ...
        rotate([0, 90, 0])
          cylinder(d = button_keycap_entry_d,
                   h = button_keycap_entry_depth + 0.08);
    }
}
...
module side_button_switch_references() {
  if (enable_side_buttons)
    at_side_button_positions() {
      ...
      // φ8 配套键帽装配参考：φ8 主帽进入整段侧壁，
      // 主帽后端与侧壁内表面对齐，外侧仅保留 button_keycap_head_h - wall 的露出量。
      color([0.08, 0.08, 0.08, 0.72])
        translate([
          side_button_inner_x - button_keycap_guide_h,
          0, 0
        ])
          rotate([0, 90, 0])
...
module screen_fpc_fixed_hook_one(x) {
  // 固定钩位于 -Y 屏幕边缘：外侧立柱与加深围边融合，顶部短唇扣住屏幕后壳。
  // 装配时不掰卡钩，而是将屏幕 FPC 侧斜插到短唇下方。
  union() {
    // 加高局部围边作为卡钩根部。
    translate([
      x - screen_hook_root_w/2,
      screen_hook_inner_y - screen_hook_base_t,
      front_thick
    ])
      cube([
        screen_hook_root_w,
...
module screen_clamp_bar_raw() {
  ...
  difference() {
    union() {
      ...
      for (x = [-screen_clamp_pad_x, screen_clamp_pad_x])
        translate([
          x,
          screen_clamp_pad_y - screen_clamp_bar_center_y,
          -screen_clamp_pad_h
        ])
          cylinder(
            d = screen_clamp_pad_d,
            h = screen_clamp_pad_h + 0.04
          );

      for (x = [-screen_cover_hook_pad_x, screen_cover_hook_pad_x])
        translate([
          x,
          screen_cover_hook_pad_y - screen_clamp_bar_center_y,
          -screen_clamp_pad_h
        ])
          cylinder(
            d = screen_clamp_pad_d,
      ...
      // FPC 转接小板的两颗无孔实心圆柱，从盖板顶面向后盖方向立起。
      for (x = [-screen_fpc_board_boss_x, screen_fpc_board_boss_x])
        translate([
          x,
          screen_fpc_board_mount_y - screen_clamp_bar_center_y,
          screen_clamp_bar_t - 0.02
        ])
          cylinder(
            d = screen_fpc_board_boss_od,
            h = screen_fpc_board_boss_h + 0.02
          );
    }
...
// ---------- 后盖 ----------
module back_cover() {
  // 后盖保留ESP32-S3四螺柱、四个边缘支撑点、Type-C侧导向块和四周弧形凸点。
  // 尾部中间不设横挡板，只在ESP32模组两侧设两块止推挡块。
  // 三枚按键已经全部迁移到前壳右侧长边。
  difference() {
    union() {
      rounded_prism(
...
module print_plate() {
  ...
  for (i = [0:1])
    translate([
      (cover_w/2 + plate_gap/2)
        + (i - 0.5) * (esp32_edge_clamp_w + 4.00),
      -(cover_h/2
        + esp32_usb_bar_h
        + esp32_edge_clamp_h/2
        + plate_gap/2 + 3.00),
      esp32_edge_clamp_toe_h
    ])
      esp32_edge_clamp_cover(i % 2 == 0 ? -1 : 1);
...
module screen_reference() {
  // 42.72 x 58.80 x 2.20mm 裸屏主体，整体向 FPC 侧偏移 1.92mm。
...
if (part == "print_plate") {
  ...
  translate([0, 0, screen_clamp_pad_h])
    screen_clamp_bar_raw();
} else if (part == "back_cover") {
  back_cover();
} else if (part == "esp32_edge_clamp_cover"
           || part == "pico_pair_clamp_cover") {
  translate([0, 0, esp32_edge_clamp_toe_h])
    esp32_edge_clamp_cover(1);
} else if (part == "esp32_usb_clamp_bar") {
  translate([0, 0, esp32_usb_bar_eva_pad_drop])
    esp32_usb_clamp_bar();
