/*
  ...
  V14修正：正面实测VA左侧黑边约1.20mm、右侧约0mm；按总差值的一半校正，
  将LCD裸屏、浅凹槽、加深限位围边、FPC根部避让及LCD保护切除整体向正面左侧移动0.60mm；
  正面可视开窗、双M2螺柱和盖板卡钩保持原位，使左右黑边理论恢复为各约0.60mm。
  ...
  V15修正：取消Type-C对侧三按键PCB上下两条限位/支撑条，仅保留两端M2螺柱支撑；避免下侧限位挡住LCD屏幕盖板装配。
  V16修正：两个Type-C外壳开孔整体向后盖方向移动1.00mm，修正当前开口偏向屏幕侧、与接口口位对不上的问题。
  ...
  V4 同步适配新的 20 x 30mm FPC 转接 PCB 四孔布局：
    ...
    13. 屏幕凹槽与裸屏在原向 FPC 侧偏移 1.92mm 的基础上再移动 1.00mm，总偏移 2.92mm；正面开窗保持原位，用于修正FPC方向的上下黑边差；
...
$fn = 56;
part = "print_plate";    // 可选："print_plate", "front_shell", "screen_clamp_bar", "screen_x_backplate", "back_cover", "esp32_usb_clamp_bar", "screen_esp32_breakaway_pair", "side_button_pcb_reference", "typec_clamp_cover", "typec_fit_test", "assembly", "exploded"
...
screen_aa_w = 37.42;
screen_aa_h = 49.66;
// V14实测推导：当屏幕玻璃/凹槽仍居中时，正面左黑边1.20mm、右黑边0mm，
// 说明该批次VA中心相对玻璃主体向+X偏约0.60mm。
screen_aa_x_offset_measured = 0.60;
...
// V14：正面观察左黑边1.20mm、右黑边0mm，中心偏差=(1.20-0)/2=0.60mm。
// 保持正面开窗X=0不动，把裸屏及其定位凹槽向正面左侧(-X)移动0.60mm。
screen_mount_x = -screen_aa_x_offset_measured; // 裸屏主体/凹槽向-X移0.60mm，抵消VA自身+X偏差
...
screen_window_x_offset = 0.00;                                      // 正面开窗X保持不动
screen_va_center_x = screen_mount_x + screen_aa_x_offset_measured;  // 校正后VA中心回到X=0
screen_va_to_window_x = screen_va_center_x - screen_window_x_offset;
...
screen_recess_left_margin =
    screen_inner_half_x + (screen_mount_x - screen_recess_w/2);
screen_recess_right_margin =
    screen_inner_half_x - (screen_mount_x + screen_recess_w/2);
screen_recess_side_margin =
    min(screen_recess_left_margin, screen_recess_right_margin);
...
screen_recess_guide_left_gap =
    screen_inner_half_x + (screen_mount_x - screen_recess_guide_outer_w/2);
screen_recess_guide_right_gap =
    screen_inner_half_x - (screen_mount_x + screen_recess_guide_outer_w/2);
screen_recess_guide_side_gap =
    min(screen_recess_guide_left_gap, screen_recess_guide_right_gap);
...
screen_window_neg_y_border =
    body_h/2 - (-screen_window_y_offset + screen_window_h/2);
screen_va_front_left_black =
    (screen_window_w - screen_aa_w)/2 + screen_va_center_x;
screen_va_front_right_black =
    (screen_window_w - screen_aa_w)/2 - screen_va_center_x;
// 加入实测VA相对玻璃主体的+0.60mm偏移后，校正结果应接近左右各0.59mm。
...
echo(str("校验：V14正面左右校正：实测VA相对玻璃X=+", screen_aa_x_offset_measured,
         "mm；裸屏/凹槽X=", screen_mount_x,
         "mm；开窗X=", screen_window_x_offset,
         "mm；理论正面左/右可见黑边约=",
         screen_va_front_left_black, "/",
         screen_va_front_right_black, "mm"));
echo(str("校验：凹槽距左/右内壁=", screen_recess_left_margin,
         "/", screen_recess_right_margin,
         "mm；加深围边距左/右内壁=",
         screen_recess_guide_left_gap, "/",
         screen_recess_guide_right_gap, "mm"));
...
assert(screen_recess_side_margin > 0.80,
       "错误：V14左右校正后，裸屏凹槽距离某一长边内壁太近");
assert(screen_recess_min_y_gap > 1.00,
       "错误：偏移后的裸屏凹槽距离某一短边内壁太近");
assert(screen_recess_guide_side_gap > -0.10,
       "错误：V14左右校正后，加深限位围边过度侵入某一长边内壁");
...
assert(abs(screen_va_to_window_x) < 0.01,
       "错误：V14加入实测VA偏移后，VA中心仍未与正面开窗中心重合");
assert(abs((1.20 - 0.00)/2 - abs(screen_mount_x)) < 0.01,
       "错误：V14左右修正量不是实测黑边差的一半");
...
typec_open_z_extra_tune = 0.00;             // Type-C 开孔高度微调；正数往后盖方向，负数往前壳方向；V16按要求相对上一版向后盖方向回移 1.00mm
...
echo(str("校验：Type-C 开孔中心Z=", typec_open_center_z, "mm；后盖内表面Z=", shell_depth - cover_t, "mm；PCB元件面Z=", pico_board_front_z, "mm；计算方式=后盖扣上后 + PCB厚 ", pico_pcb_t, "mm + Type-C半高 ", typec_shell_h_for_open_calc/2, "mm；V16相对上一版向后盖方向回移1.00mm"));
...
// V15：取消按键PCB上下限位/支撑条，避免下侧限位挡住LCD屏幕盖板。
enable_side_button_edge_rails = false;
...
echo(str("校验：PCB元件面Y=", side_button_pcb_component_y,
         ...
         2*side_button_pcb_screw_x, "mm；上下限位/支撑条=",
         enable_side_button_edge_rails ? "启用" : "关闭"));
...
module side_button_edge_support_rails() {
  if (enable_side_buttons && enable_side_button_edge_rails)
    for (sz = [-1, 1]) {
...
module screen_recess_guide_wall() {
  if (enable_screen_recess_guide) {
    translate([screen_mount_x, screen_mount_y, front_thick])
      difference() {
        rounded_prism(
          screen_recess_guide_outer_w,
          screen_recess_guide_outer_h,
    ...
    union() {
      difference() {
        ...
        rounded_cut(screen_window_w, screen_window_h,
                    ...
                    screen_window_x_offset, screen_window_y_offset, -0.60);
...
// ---------- 03 + 04 易折连体打印件 ----------
// 用途：部分打印平台会拒绝过小的独立零件（esp32_usb_clamp_bar）。
// 将 04 放在 03 下方，并通过两条薄连接筋组成一个连通实体；打印后可直接手掰/剪断。
breakaway_gap = 1.20;           // 03 与 04 主体之间净间隙
breakaway_bridge_w = 2.20;      // 单条连接筋 X 向宽度
breakaway_bridge_t = 0.60;      // 连接筋厚度；0.4喷嘴约1~2层，易折
breakaway_bridge_overlap = 0.25;// 两端吃入零件的重叠量，保证布尔连通
breakaway_bridge_x = 13.00;     // 两条筋中心X；避开03中部FPC开口

module screen_esp32_breakaway_pair() {
  esp_pair_y = -(screen_clamp_bar_h/2 + breakaway_gap + esp32_usb_bar_h/2);
  bridge_y = -(screen_clamp_bar_h/2 + breakaway_gap/2);
  bridge_len = breakaway_gap + 2*breakaway_bridge_overlap;

  union() {
    // 03：屏幕异形盖板，保持原打印方向，底面Z=0。
    screen_clamp_bar_raw();

    // 04：ESP32 EVA横压板，保持原打印方向，底面Z=0。
    translate([0, esp_pair_y, 0])
      esp32_usb_clamp_bar();

    // 两条薄桥：避开03中央FPC槽，同时连接到04的31mm主体横梁。
    for (x = [-breakaway_bridge_x, breakaway_bridge_x])
      translate([
        x - breakaway_bridge_w/2,
        bridge_y - bridge_len/2,
        0
      ])
        cube([
          breakaway_bridge_w,
...
module print_plate() {
  ...
  // 03+04：使用易折筋连成一个整体，避免04因尺寸过小被打印平台拒绝。
  translate([
    cover_w + plate_gap
...
module screen_reference() {
  // 42.72 x 58.80 x 2.20mm 裸屏主体，当前向FPC侧偏移2.92mm，并向正面左侧偏移0.60mm。
  ...
  // VA有效显示区加入实测+X 0.60mm玻璃内偏差；主体左移后，VA在X方向重新对中。
...
if (part == "print_plate") {
  ...
  // V11：横压板平底，Z=0直接导出。
  esp32_usb_clamp_bar();
} else if (part == "screen_esp32_breakaway_pair") {
  // 03+04易折连体导出：用于打印平台最小零件限制。
  screen_esp32_breakaway_pair();
