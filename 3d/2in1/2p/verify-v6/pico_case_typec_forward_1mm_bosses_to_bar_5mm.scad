/*
  ...
  本版按你发的屏幕图纸重新校准：
    - 本次修改：Type-C 开孔朝前壳方向移动 1.00mm；后盖两对 PCB 固定螺柱整体朝 PCB 底部横向挡板方向平移 5.00mm。
...
module at_pico_support_pads() {
  ...
  for (x = [-pico_support_pad_x, pico_support_pad_x]) {
    translate([pico_center_x + x, pico_center_y + pico_support_pad_y_top + pico_mount_boss_shift_y, 0]) children();
    translate([pico_center_x + x, pico_center_y + pico_support_pad_y_bottom + pico_mount_boss_shift_y, 0]) children();
  }
}
