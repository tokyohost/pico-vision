/*
  ...
  Revision:
    ...
    - Rear cover has NO screws and NO rigid snap tabs. It uses tiny arc-shaped locating beads plus matching shallow scoops.
    ...
    - Shell top/opposite short side has a rounded USB Type-C opening.
    - Front border is kept narrow by removing rear-cover screw posts and snap structures.
    - Avoids forced interference deformation; the beads are lower than the cover side clearance and only locate into shallow recesses.

  Coordinate system:
    X = screen width
    Y = screen height; positive Y is the USB-C side after moving Type-C to the opposite short edge
    Z = front -> rear

  Export:
    Change `part` below to:
      "print_plate" : front_shell and back_cover laid flat side-by-side on the same Z=0 print plane
      "front_shell" : front frame/tray, screen four bosses, opposite-side Type-C opening, half-thickness rear rabbet
  ...
  Print suggestion:
    ...
    - Rear cover adds tiny arc beads and the front shell has matching shallow arc scoops; this is detent-like locating, not forced press-fit.
...
// ---------- Arc locating beads / matching shallow scoops ----------
// This is NOT a forced press-fit. The front shell has open shallow guide scoops at the same positions,
// so the cover beads have a path to enter instead of forcing the shell wall to flex.
// With cover_clearance = 0.40, side clearance is about 0.20 mm per side.
// detent_bead_out = 0.22 mm is only 0.02 mm over the normal side clearance,
// and the matching guide/scoop removes that interference locally.
enable_arc_detents = true;
detent_bead_out = 0.22;          // protrusion beyond rear-cover edge; set 0.18 for looser/no-contact assembly
...
detent_guide_depth = 0.32;       // open guide depth cut in front-shell rear rabbet wall; prevents forced deformation
detent_pocket_depth = 0.36;      // final shallow arc scoop depth; slightly deeper than the guide
detent_pocket_extra = 1.20;      // scoop / guide is slightly longer than bead for easy alignment
...
// Type-C has been moved to the opposite short edge: positive-Y side.
// Set pico_usb_side_y = -1 if you ever want to move it back to the original negative-Y side.
// IMPORTANT:
...
pico_usb_side_y = 1;              // 1 = positive-Y opposite side, -1 = original negative-Y side
...
// Positive value means the connector mouth protrudes outward past the Pico USB-side edge.
...
module at_pico_tail_holes() {
  // Tail holes are on the side opposite the USB-C edge.
...
module guide_slot_long_side(sx, y) {
  // Open shallow guide from the rear mouth down to the cover seat.
  // This lets the arc bead enter through a relieved path instead of bending the shell wall.
  guide_len = detent_bead_len + detent_pocket_extra;
  guide_r = min(0.35, min(detent_guide_depth, guide_len)/2 - 0.01);
  guide_z0 = shell_depth - cover_t - 0.12;
  guide_h = cover_t + 0.32;
  rounded_cut(detent_guide_depth, guide_len, guide_h, guide_r,
              sx * (rear_rebate_w/2 + detent_guide_depth/2 - detent_side_overlap),
              y,
              guide_z0);
}

module guide_slot_short_side(sy, x) {
  guide_len = detent_bead_len + detent_pocket_extra;
  guide_r = min(0.35, min(detent_guide_depth, guide_len)/2 - 0.01);
  guide_z0 = shell_depth - cover_t - 0.12;
  guide_h = cover_t + 0.32;
  rounded_cut(guide_len, detent_guide_depth, guide_h, guide_r,
              x,
              sy * (rear_rebate_h/2 + detent_guide_depth/2 - detent_side_overlap),
              guide_z0);
}
...
module front_shell_arc_scoop_cuts() {
  if (enable_arc_detents) {
    // Long-edge open guides plus final arc scoops.
    for (sx = [-1, 1])
      for (yf = [-detent_long_y_frac, detent_long_y_frac]) {
        guide_slot_long_side(sx, yf * cover_h);
        arc_scoop_long_side(sx, yf * cover_h);
      }

    // Short-edge open guides plus final arc scoops.
    if (enable_short_edge_detents)
      for (sy = [-1, 1]) {
        guide_slot_short_side(sy, 0);
...
module front_shell() {
  difference() {
    ...
    // Type-C side opening on the opposite short edge. Cut through shell and shelf if they overlap.
    side_typec_cut(typec_open_w, typec_open_h, typec_cut_depth, typec_open_r,
                   typec_open_x, typec_open_y, typec_open_center_z);

    // Shallow concave arc scoops for the rear-cover locating beads.
...
module back_cover() {
  difference() {
    union() {
      ...
      // Tiny arc-shaped locating beads. They are lower than side clearance, so insertion should not visibly deform the shell.
...
module assembled_back_cover() {
  ...
  // Use Y-axis flip instead of X-axis flip so the Pico USB-C side keeps its Y side,
  // matching the front-shell opposite-side Type-C opening after assembly.
...
if (part == "print_plate") {
  ...
  // Type-C opening reference on the opposite short edge.
