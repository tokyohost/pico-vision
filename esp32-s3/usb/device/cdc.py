...
class CDCInterface(io.IOBase, Interface):
    ...
    """实现保留 REPL 之外的独立 USB CDC ACM 数据接口。"""
    def __init__(self, **kwargs):
        # io.IOBase has no __init__()
        """初始化 CDC 控制状态、端点与收发缓冲区。"""
    ...
    def any(self):
        """返回接收缓冲区中无需等待即可读取的字节数。"""
        return self._rb.readable()
