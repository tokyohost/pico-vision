...
class Application:
    ...
    def __init__(self, protocol, transport=None):
        ...
        self._render_max_regions = RENDER_MAX_REGIONS
        self._render_time_budget_us = RENDER_TIME_BUDGET_US
        self._led = create_led_controller()
        self._led.start()
        self._protocol.write(
            "BOOT:BOARD_MODEL:{}\n".format(
                "esp32-s3"
    ...
    def _return_to_waiting_page(self):
        ...
        # Monitor 进程退出只会结束主机侧会话，ESP32-S3 内置 USB 控制台仍然
        # 保持可用；继续轮询原协议即可在 Monitor 重启后自动恢复。
        self._next_boot_animation = time.ticks_ms()
    ...
    def run(self):
        ...
        self._protocol.write(b"BOOT:ESP32_S3_LCD_READY\n")
        self._show_boot(
            100,
            "BOOT:ESP32_S3_LCD_READY",
            "waiting connecting ....",
            flush=True,
        )
        while True:
