...
class LcdPinProfile:
    ...
    def __init__(
        ...
        self.dc = int(dc)
        self.rst = int(rst)
        if not isinstance(backlight, LcdBacklightProfile):
            raise TypeError("背光参数必须是 LcdBacklightProfile 实例")
...
def create_eight_pin_profile():
    """创建 ESP32-S3 八针屏固定脚位档案。"""
