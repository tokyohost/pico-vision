...
| --- | --- | --- | --- | --- |
| `default` | `style_default.py` | 竖屏 240×320 | 原生字体 | 通用系统概览 |
| `disk` | `style_disk.py` | 竖屏 240×320 | 原生字体 | 突出磁盘总容量和网络趋势 |
| `horizontal_disk` | `style_horizontal_disk.py` | 横屏 320×240 | 二寸屏字体 | 高密度显示最多九块磁盘 |
| `horizontal_disk4x` | `style_horizontal_disk4x.py` | 横屏 320×240 | 紧凑字体 | 清晰显示最多四块磁盘 |
| `horizontal_disk4x_qb` | `style_horizontal_disk4x_qb.py` | 横屏 320×240 | 紧凑字体 | 四磁盘与 qBittorrent 状态 |
| `horizontal_disk6x` | `style_horizontal_disk6x.py` | 横屏 320×240 | 紧凑字体 | 均衡显示最多六块磁盘 |
| `simple` | `style_simple.py` | 横屏 320×240 | 紧凑字体 | 健康优先的三磁盘简洁视图 |
...
新增样式时使用 `style_<名称>.py` 命名，并在模块末尾调用 `register_style()` 注册。
完整插件接口和脏区域刷新约定请参阅 `STYLE_PLUGIN.md`。若样式需要由 Monitor 命令行选择，
还需将选择名称加入 `monitor/pico_monitor.py` 的 `BUILTIN_LCD_STYLES`。
