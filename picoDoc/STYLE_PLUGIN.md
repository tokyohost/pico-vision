...
6. 样式的 `font_name` 定义该样式自己的默认字体。可选择 `native`、
   `screen_2inch`、`screen_2inch_compact`、`wqy_8x16`、
   `fusion_pixel_8x16` 或 `fusion_pixel_8px`。其中两个 8×16 字体来自已编译
   双语字模的 MicroPython 固件，`fusion_pixel_8px` 来自运行时已加载的字库文件。
...
```text
ACK:LCD_FRAME:<version>:TOTAL=...MS:CANVAS=...US:LCD=...US:REGIONS=...:MEMORY_USED=...:MEMORY_TOTAL=...:CANVAS_BACKEND=...:PROTOCOL_BACKEND=...
```

- `CANVAS` 包含样式计算、文字、曲线和 Canvas 绘制时间。
- `CANVAS_BACKEND` 为 `C` 时表示使用 UF2 原生加速，为 `PYTHON` 时表示使用兼容回退实现。
- `PROTOCOL_BACKEND` 为 `C` 时表示 PV1 帧使用 UF2 原生解析器，为 `PYTHON` 时表示使用 Python 回退解析器。
- `LCD` 是将脏区域写入屏幕的时间。
...
单次文字可以通过 `font_name` 临时选择 SDK 已编译或运行时已加载的字体，宽度计算必须
传入相同名称；省略参数时继续使用当前样式设置的字体，样式未设置时默认为 `native`。
单次调用结束后会自动恢复样式字体，不会影响后续文字：

```python
canvas.text(8, 20, "系统正常", WHITE, font_name="wqy_8x16")
width = canvas.text_width("系统正常", font_name="wqy_8x16")
canvas.text(8, 40, "Fusion Pixel", WHITE, font_name="fusion_pixel_8x16")
canvas.text(8, 60, "文件字库", WHITE, font_name="fusion_pixel_8px")
```
