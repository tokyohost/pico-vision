...
7. ESP32-S3 样式可声明 `sync_visible_frame_to_second = True`，让当前样式的
   最新待显示帧由 `fn_lcd` 原生异步任务在下一整秒按脏区自动提交。新帧只覆盖
   尚未显示的旧待显示帧，不会阻塞 Python 渲染线程。FPS、游戏等高帧率样式应
   设为 `False`；未声明的旧自定义样式默认关闭。
...
```text
ACK:LCD_FRAME:<version>:TOTAL=...MS:CANVAS=...US:LCD=...US:REGIONS=...:MEMORY_USED=...:MEMORY_TOTAL=...:CANVAS_BACKEND=...:PROTOCOL_BACKEND=...:LCD_BACKEND=...
```
...
- `LCD_BACKEND` 为 `NATIVE_DMA` 时表示使用内部 DMA 双缓冲，为 `LEGACY` 时表示使用标准 SPI 兼容路径。
- `LCD` 是将脏区域写入屏幕的时间。
...
单次文字可以通过 `font_name` 临时选择 SDK 已编译的字体，宽度计算必须
