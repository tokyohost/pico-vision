...
def validate_script(script_path, existing_keys=None):
    """校验自定义数据脚本格式、key 唯一性和返回值类型。"""
    import re
...
class CustomDataManager:
        ...
        with self.lock:
            ...
            for script_path in sorted(self.custom_directory.glob("*.py")):
                if script_path.name == CUSTOM_DATA_TEMPLATE_NAME:
                    continue
                try:
                    definition = validate_script(script_path, existing_keys=set(definitions))
            ...
            for key, definition in definitions.items():
                ...
                if old_state and old_state.definition.path == definition.path:
                    ...
                    self.states[key] = old_state
                else:
                    self.states[key] = CustomDataState(definition=definition)
    ...
    def import_script(self, source_path):
        ...
        definition = validate_script(source_path, existing_keys=existing_keys)
        ...
        if target_path.resolve() != source_path:
            ...
            definition = validate_script(target_path, existing_keys=existing_keys)
        self.reload_scripts()
        return definition
    ...
    def test_script(self, script_path):
        ...
        try:
            ...
            definition = validate_script(script_path, existing_keys=keys)
