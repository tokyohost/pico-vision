...
def create_argument_parser(config=None):
    ...
    usb_cdc_group = parser.add_mutually_exclusive_group()
    usb_cdc_group.add_argument("--force-usb-cdc", dest="force_usb_cdc", action="store_true", help="强制使用 USB-CDC，并停止 WebSocket 直连与局域网主动搜索")
    usb_cdc_group.add_argument("--no-force-usb-cdc", dest="force_usb_cdc", action="store_false", help="允许按已保存地址连接或主动搜索 WebSocket 设备")
    ...
    idle_group.add_argument("--idle-enabled", dest="idle_enabled", action="store_true", help="允许 Pico 在空闲超时后进入待机样式")
    idle_group.add_argument("--no-idle", dest="idle_enabled", action="store_false", help="禁止 Pico 进入待机样式")
    parser.set_defaults(idle_enabled=config_flag(config, "PICO_MONITOR_IDLE_ENABLED", True))
    parser.add_argument("--idle-style", default=config_value(config, "PICO_MONITOR_IDLE_STYLE", "idle"), help="Pico LCD 待机样式名称")
    parser.add_argument("--idle-timeout", type=int, default=int(config_value(config, "PICO_MONITOR_IDLE_TIMEOUT", "30")), help="连续未收到 JSON 后进入待机的秒数")
    ...
    parser.add_argument("--upgrade-pico", action="store_true", help="下载当前 Monitor 版本的 Pico 全量固件包并执行更新")
