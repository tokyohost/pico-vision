...
class WifiCommandMixin:
    ...
    def request_wifi_list(self, result_queue=None):
        """安排主循环在设备通信空闲时扫描附近无线网络。"""
        self.wifi_operations.put(("list", None, result_queue))

    def request_wifi_connect(self, payload, result_queue=None):
        """校验并安排连接指定无线网络。"""
    ...
    def request_wifi_forget(self, payload, result_queue=None):
        """校验并安排忘记指定的已保存无线网络。"""
        if not isinstance(payload, dict):
        ...
        if not isinstance(ssid, str) or not ssid.strip():
            raise ValueError("Wi-Fi 名称不能为空")
        self.wifi_operations.put(("forget", {"ssid": ssid.strip()}, result_queue))
    ...
    def publish_wifi_operation(self):
        ...
        operation = self.wifi_operations.get_nowait()
        action, payload = operation[:2]
        result_queue = operation[2] if len(operation) > 2 else None
        try:
            if action == "list":
