...
CPU 数据通过顶层 `cpu` 对象发送，其中 `frequency_ghz` 是当前 GHz 频率。GPU 数据通过顶层 `gpu` 对象发送，除使用率和历史外，还包含专用显存 `dedicated_memory_used_bytes`/`dedicated_memory_total_bytes` 与温度 `temperature_c`；操作系统或驱动不提供对应指标时字段值为 `null`。
