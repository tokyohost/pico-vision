...
CPU 数据通过顶层 `cpu` 对象发送，其中 `frequency_ghz` 是当前 GHz 频率。GPU 数据通过顶层 `gpu` 对象发送，除使用率和历史外，还包含专用显存 `dedicated_memory_used_bytes`/`dedicated_memory_total_bytes` 与温度 `temperature_c`；操作系统或驱动不提供对应指标时字段值为 `null`。Linux 支持 NVIDIA NVML、AMD sysfs 和 Intel i915 PMU；Intel 核显使用共享内存，因此专用显存字段通常为 `null`。DEB 服务默认以 root 身份运行，可直接读取 i915 系统级 PMU；手动以普通用户运行时，需要系统允许该用户访问对应 perf 事件。
...
当设备管理没有已连接设备，或设备已人工进入 ROM 下载模式时，可使用“强刷 SDK”。该入口仍会先校验 `.bin` 镜像，但由用户手动选择目标 COM 口，并让 esptool 通过默认复位策略尝试进入下载模式；使用前必须确认所选 COM 口属于目标 ESP32-S3 设备。
