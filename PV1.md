...
当前 Pico 每轮最多读取 4096 字节。接收缓存上限取以下较大值并预留一个传输块：
...
```json
{"mode":"snapshot","request_id":1,"data":{"host":"示例主机","display":{"dev":false}}}
```
...
固件返回 `BAD_FRAME_LENGTH` 时附带不包含业务载荷的长度现场，例如：

```text
BAD_FRAME_LENGTH:DECLARED=1772:REMAINDER=1771:SHORTAGE=1:MAX=16384:LINE_BYTES=1791:READS=4:BACKEND=PYTHON
```

`DECLARED` 是帧头声明的载荷长度，`REMAINDER` 是第四个冒号后实际收到的总字节数，
