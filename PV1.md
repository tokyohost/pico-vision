...
3. 消息层：定义 `PING`、`PONG`、`JSONZ`、`ACK`、`ERR`、`EVENT`、`UPGRADE` 和 `STATUS`。
...
ACK:LCD_FRAME:<version>:TOTAL=...MS:...
...
载荷是 ASCII 升级子命令，不包含旧版 `UPGRADE:` 前缀。支持：

```text
BEGIN:<version>:<file_count>
FILE:<path>:<size>:<sha256>
DATA:<sequence>:<base64_data>
FILE_END
COMMIT
ABORT
```

升级文件数据在 `DATA` 子命令内使用 Base64。每个升级命令本身仍由 PV1 长度和 CRC 保护。
